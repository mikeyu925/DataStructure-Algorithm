自己还差的很远，就离谱，哪些第一名第二名，是怎么做到3分钟就写一道题的？？？
...
```java
public class Solution {
    int [][] cnt2_5 = new int[1005][2];
    /**
     * 初始化cnt2_5，求数据范围内每个数的因数2和5的个数
     */
    private void initCnt2_5(){
        for (int i = 2;i <= 1000;i++){
            if (i % 2 == 0) cnt2_5[i][0] = cnt2_5[i/2][0] + 1;
            if (i % 5 == 0) cnt2_5[i][1] = cnt2_5[i/5][1] + 1;
        }
    }
    int [][][] rowsCnt; // rowsCnt[i][j][0] 代表第i行前j个数的2的因子的个数和...
    int [][][] colsCnt; // colsCnt[j][i][0] 代表第i列前j个数的2的因子的个数和...
    public int maxTrailingZeros(int[][] grid) {
        initCnt2_5();
        int m = grid.length;  // m行
        int n = grid[0].length; // n列
        // 初始化rowsCnt 利用了前缀和
        rowsCnt = new int[m+1][n+1][2];
        for (int i = 0;i < m;i++){
            rowsCnt[i][0][0] = cnt2_5[grid[i][0]][0];
            rowsCnt[i][0][1] = cnt2_5[grid[i][0]][1];
        }
        for (int i = 0;i < m;i++){
            for (int j = 1;j < n;j++){
                rowsCnt[i][j][0] = rowsCnt[i][j-1][0] + cnt2_5[grid[i][j]][0];
                rowsCnt[i][j][1] = rowsCnt[i][j-1][1] + cnt2_5[grid[i][j]][1];
            }
        }
        // 初始化colCnt 利用了前缀和
        colsCnt = new int[m][n][2];
        for (int i = 0;i < n;i++){
            colsCnt[0][i][0] = cnt2_5[grid[0][i]][0];
            colsCnt[0][i][1] = cnt2_5[grid[0][i]][1];
        }
        for (int i = 0;i < n;i++){
            for (int j = 1;j < m;j++){
                colsCnt[j][i][0] = colsCnt[j-1][i][0] + cnt2_5[grid[j][i]][0];
                colsCnt[j][i][1] = colsCnt[j-1][i][1] + cnt2_5[grid[j][i]][1];
            }
        }
        ...
        for (int i = 0;i < m;i++){
            for (int j = 0;j < n;j++){
                // 当前点的2和5因数的个数
                int now2 = cnt2_5[grid[i][j]][0],now5 = cnt2_5[grid[i][j]][1];
                int [][] dirscnt = new int[4][2];  // 左上右下 0123
                if (j != 0){ // 当前点左边的2 和 5 的个数
                    dirscnt[0][0] = rowsCnt[i][j-1][0];
                    dirscnt[0][1] = rowsCnt[i][j-1][1];
                }
                if (i != 0){ // 当前点上边的2 和 5 的个数
                    dirscnt[1][0] = colsCnt[i-1][j][0];
                    dirscnt[1][1] = colsCnt[i-1][j][1];
                }
                if (j != n-1){ // 当前点右边的2 和 5 的个数
                    dirscnt[2][0] = rowsCnt[i][n-1][0] - rowsCnt[i][j][0];
                    dirscnt[2][1] = rowsCnt[i][n-1][1] - rowsCnt[i][j][1];
                }
                if (i != m-1){ // 当前点下边的2 和 5 的个数
                    dirscnt[3][0] = colsCnt[m-1][j][0] - colsCnt[i][j][0];
                    dirscnt[3][1] = colsCnt[m-1][j][1] - colsCnt[i][j][1];
                }
                // 左上 路线
                ans = Math.max(ans,Math.min(dirscnt[0][0] + dirscnt[1][0] + now2,dirscnt[0][1]+dirscnt[1][1]+now5));
                // 右上路线
                ans = Math.max(ans,Math.min(dirscnt[2][0] + dirscnt[1][0] + now2,dirscnt[2][1] + dirscnt[1][1]+now5));
                // 右下 路线
                ans = Math.max(ans,Math.min(dirscnt[2][0] + dirscnt[3][0] + now2,dirscnt[2][1] + dirscnt[3][1]+now5));
                // 左下 路线
                ans = Math.max(ans,Math.min(dirscnt[0][0] + dirscnt[3][0] + now2,dirscnt[0][1] + dirscnt[3][1]+now5));
            }
        }
        return ans;
    }
...
通过测试用例，全部用例出现 Wrong Answer  版本代码
...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        if (minNeed > x){
            ...
            int sec = 0;
            while (r < n){
                int r_limit = Math.min(2*k-2 + sec * k ,n-1);
...
<img src="有趣的竞赛.assets/1200px-telephone-keypad2svg.png" alt="img" style="zoom: 25%;" />
...
![image-20220508121227216](有趣的竞赛.assets/image-20220508121227216.png)
...
class Solution {
    public int countTexts(String pressedKeys) {
        ...
        dp[7][4] = 8;
        for (int j = 5;j <= n;j++){
            dp[7][j] = (dp[7][j-1] + dp[7][j-2] + dp[7][j-3] + dp[7][j-4]) % MOD;
        }
        dp[8][1] = 1;
        dp[8][2] = 2;
        dp[8][3] = 4;
        for (int j = 4;j <= n;j++){
            dp[8][j] = (dp[8][j-1] + dp[8][j-2] + dp[8][j-3]) % MOD;
        }
        dp[9][1] = 1;
        dp[9][2] = 2;
        dp[9][3] = 4;
        dp[9][4] = 8;
        for (int j = 5;j <= n;j++){
            dp[9][j] = (dp[9][j-1] + dp[9][j-2] + dp[9][j-3] + dp[9][j-4]) % MOD;
        }
        List<Long> cnts = new ArrayList<>();
        int l = 0,r = 0;
        while (r < n){
            char c = pressedKeys.charAt(r);
            while (r < n && pressedKeys.charAt(r) == c){
                r += 1;
            }
            cnts.add(dp[c-'0'][r-l]);
            l = r;
...
<img src="有趣的竞赛.assets/2022-05-08_121842.jpg" alt="2022-05-08_121842" style="zoom:50%;" />
...
![img](有趣的竞赛.assets/example2drawio.png)
...
1. **优化1**：用一个变量 cc 表示括号字符串的平衡度：遇到 $'('$ 就 +1，遇到 $')'$ 就 −1。那么合法字符串等价于任意时刻 $c\ge 0 且最后 c=0$。

   - **剪枝**：当某个时刻$c < 0 或者 c > (n+m-1) / 2$时，直接剪枝返回false。

     > c > (n+m-1) / 2 表示就算后面全是 ) 也不满足条件

2. **优化2**：把进入格子**前**的 c*c* 值当作格子的附加状态，那么一个格子至多有 $(m+n-1)/2$ 种不同的状态，整个网格图至多有$ mn(m+n-1)/2$ 个不同的状态。
...
<img src="有趣的竞赛.assets/image-20220508141048899.png" alt="image-20220508141048899" style="zoom:50%;" />
...
![img](有趣的竞赛.assets/example1drawio3.png)
...
我这个双指针写的不是很好，有点不好看懂，不过时间和空间都是100%

<img src="有趣的竞赛.assets/image-20220515222338058.png" alt="image-20220515222338058" style="zoom:67%;" />

大体意思就是不断地更新右边界，然后判断新更新了右边界后可以新+多长地瓷砖，因为每次更新了右边界，左边界也是变大的，因此要将左边界左边的瓷砖（窗口外的瓷砖长度减去）
...
class Solution {
    public int maximumWhiteTiles(int[][] tiles, int carpetLen) {
        ...
        for (int i = 0;i < n;i++){
            pairs.add(new Pair(tiles[i][0],tiles[i][1]));
        }
        ...
        int len = 0;
        for (int i = 0;i < n;i++){
            Pair top = pairs.get(i);
            int r_limit = top.l + carpetLen - 1;
            while (idx < n && r_limit >= pairs.get(idx).r){
                len += pairs.get(idx).r - pairs.get(idx).l + 1;
                idx += 1;
            }
            int tmp = 0;
            if (idx < n && r_limit >= pairs.get(idx).l){
                tmp = r_limit - pairs.get(idx).l + 1;
                len += tmp;
            }
            ans = Math.max(ans,len);
            if (len >= top.r - top.l + 1){
                len -= top.r - top.l + 1;
                len -= tmp;
            }else{
                len -= len;
...
统计每一位有多少个数在该位是1，个数最多的哪一位的个数就是答案
...
根据分数的公式：$Sum * len$，可以想到需要先求 前缀和 ，方便快速地计算某个区间的和。

然后我们采用双指针，不断移动双指针的右指针知道区间的 **分数** 大于等于 k，此时计算该区间有多少个符合条件的子数组。

> 注意：可能会存在重复计算的情况，因此要将重复的删除

![image-20220612095358019](有趣的竞赛.assets/image-20220612095358019.png)
...
给你两个字符串 s 和 sub 。同时给你一个二维字符数组 mappings ，其中 $mappings[i] = [old_i, new_i]$ 表示你可以替换 sub 中任意数目的 $old_i$ 个字符，替换成 $new_i$ 。sub 中每个字符 **不能** 被替换超过一次。
...
![image-20220612102108478](有趣的竞赛.assets/image-20220612102108478.png)

> 感觉思路是对的，但是就是不能完全通过...  
>
> 出个小考验：哪位兄弟有能找出来Bug？

![image-20220612100703998](有趣的竞赛.assets/image-20220612100703998.png)
    ...
    public long distinctNames(String[] ideas) {
        ...
        for (int i = 0;i < n;i++){
            for (int j = 0;j < n;j++){
                ...
                String na = a.substring(0,1) + b.substring(1);
                if (names.contains(na)) continue;
                String nb = b.substring(0,1) + a.substring(1);
                if (names.contains(nb)) continue;
                ans += 1;
            }
        }
        return ans;
...
可以看到，如果尝试枚举两个字符串，时间复杂度达到$O(n^2)$，因此想着尝试能否只枚举一个字符串，然后快速计算另外一个字符串。

首先将原字符串按照首字母分为26类（因为只有小写字母）

定义$f(i,j)$为有多少个首字母为 i 的字符串，将首字母替换成 j 后，未在 **ideas** 中出现过。

当我们计算出来了所有的 $f(i,j)$后，如果$f(j,i)$也成立，那么就将$f(j,i)$累加至答案。
...
class Solution {
    public long distinctNames(String[] ideas) {
        ...
        for (String s : ideas){
            int a = s.charAt(0) - 'a';
            for (int i = 0;i < 26;i++){ // 判断下将首字母换成i，有没有在元字符串中出现过
                StringBuilder sb = new StringBuilder();
                sb.append((char)(i+'a'));
                sb.append(s.substring(1));
                String t = sb.toString();
                if (!hash.contains(t)) dp[a][i] += 1;
            }
        }
        long ans = 0;
        for (String s : ideas){
...
**里程悲：第5次参加LC周赛经典秒三道后第四道坐大牢，是时候开始狂刷困难题了。**
...
![img](./有趣的竞赛.assets/agraphdrawio.png)
...
<img src="./有趣的竞赛.assets/image-20221218151851113.png" alt="image-20221218151851113" style="zoom:50%;" />

```cpp
class Solution {
public:
    bool isPossible(int n, vector<vector<int>>& edges) {
        vector<unordered_set<int>> mmap(n+1);
        for(auto & edge : edges){
            int a = edge[0];
            int b = edge[1];
            mmap[a].insert(b);
        ...
        if(odds.size() == 0){
            ...
            int a = odds[0];
            int b = odds[1];
            if(mmap[a].count(b) == 0){ // a b 之间没有连线
                return true;
            }else{
                for(int i = 1;i <= n;i++){
                    if(i != a && i != b && (!mmap[i].count(a)) && (!mmap[i].count(b))){
                        return true;
                    }
                }
                return false;
            ...
            if(mmap[a].count(b) == 0 && mmap[c].count(d) == 0) return true;
            if(mmap[a].count(c) == 0 && mmap[b].count(d) == 0) return true;
            if(mmap[a].count(d) == 0 && mmap[c].count(b) == 0) return true;
        }
