...
对于每一个查询，我们将值较小的设为a，较大的设为b。

首先，由于编号为 `val` 的节点都有两个子节点，满足：

- 左子节点的编号为 `2 * val`
- 右子节点的编号为 `2 * val + 1`

根据这个关系，从各自结点向上走，找两者的LCA「最近公共祖先结点」

则两者形成的还的长度 len 即：
$$
len = dia_A - dis_{Con} + dis_B + 1
$$
公式中「变量」含义如下图所示：

<img src="./二叉树.assets/image-20221218115703837.png" alt="image-20221218115703837" style="zoom:50%;" />
